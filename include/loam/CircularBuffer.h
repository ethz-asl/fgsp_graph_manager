...
class CircularBuffer {
 public:
  CircularBuffer(const size_t &capacity = 50)
      : _capacity(capacity),
        _size(0),
        _startIdx(0) {
  ...
  void push(const T &element) {
    ...
    else
    {
      ...
      _startIdx = (_startIdx + 1) % _capacity;  //smk:resets _startIdx to 0 if it equals _capacity
    }
  }

  //CUTOMIZATION
  /** \brief Returns Index of last inserted element in the buffer.
   *
   * @return buffer index of last inserted element
   */
  size_t getLastIdx() {
    size_t idx = _size == 0 ? 0 : (_startIdx + _size - 1) % _capacity;
    return idx;
  ...
  /** \brief Returns Index of first inserted element in the buffer.
   *
   * @return buffer index of first inserted element
   */
  size_t getStartIdx() {
    return _startIdx;
  }
